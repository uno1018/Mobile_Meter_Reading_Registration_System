...
Run `supabase/installation_request.sql` in each Water District project. It creates `installation_request`, keeps `updated_at` server side with a trigger, and restricts access: devices may insert a PENDING request with the anon key, only registration administrators may read the table or change a status, and a device reads back its own status through `get_registration_status(device_id)`.
